 ...
 * @param {number} [options.lightPower=2500] - 光源功率
 * @param {number} [options.ambientColor=0x4466ff] - 环境光颜色
 * @param {number} [options.bgColorTop=0x66bbff] - 渐变顶部颜色
 * @param {number} [options.bgColorBottom=0x4466ff] - 渐变底部颜色
...
function initSkinning(containerId, modelConfig, options) {
    ...
    var opts = Object.assign({
        ...
        lightPower: 2500,
        ambientColor: 0x4466ff,
        bgColorTop: 0x66bbff,
        bgColorBottom: 0x4466ff,
    }, options);
    ...
    // TSL 渐变背景
    if (tsl.color && tsl.screenUV && scene.backgroundNode !== undefined) {
        try {
            scene.backgroundNode = tsl.screenUV.y.mix(tsl.color(opts.bgColorTop), tsl.color(opts.bgColorBottom));
        } catch (e) {
            scene.background = new T.Color(opts.bgColorBottom);
        }
    } else {
        scene.background = new T.Color(opts.bgColorBottom);
    }
